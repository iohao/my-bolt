 ...
 */
@Slf4j
public class ExternalClientMyResponseProcessor extends AsyncUserProcessor<MyResponse> {

    @Override
    public void handleRequest(BizContext bizCtx, AsyncContext asyncCtx, MyResponse myResponse) {
        if (MyBoltHelper.openLog) {
            log.info("步骤-4 ---- 请求到达 external -- {}", myResponse);
        }

        // 统计次数
        MyBoltHelper.countRequest.increment();
    }
